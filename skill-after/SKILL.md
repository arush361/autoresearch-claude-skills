...
### Projected Totals (MANDATORY)

Even though 4 criteria require user input, you MUST ALWAYS:

1. **Calculate projected WINNING totals /60** using conservative placeholder scores of 6/10 for each user-scored criterion (clearly labeled as "illustrative, assuming average user scores of 6/10"). Show the projected total for every gap/feature.
2. **Provide a preliminary FILE/WAIT/SKIP recommendation** based on the projected total. Clearly state this is preliminary and will be finalized after the user provides their scores.
3. **Include a Decision Summary table** at the end of every gap analysis, PRD, or backlog output with projected totals and preliminary recommendations.

This applies to ALL output types:
- **Gap analysis (`/pm:gaps`)**: Project totals for each gap, provide preliminary FILE/WAIT/SKIP per gap
- **PRD (`/pm:prd`)**: Project the total for the feature, state preliminary recommendation
- **Backlog (`/pm:backlog`)**: Project totals for each item, assign preliminary Now/Next/Later buckets
- **Landscape (`/pm:landscape`)**: After competitor profiles, include a "Preliminary Gap Signals" section with WINNING projections for 3-5 obvious gaps spotted during research, using the hybrid scoring format (Claude scores Pain/Timing with evidence, marks user criteria as "projected 6/10", calculates projected total)
- **Product analysis (`/pm:analyze`)**: After inventory, include a "WINNING Preview" section scoring 3-5 likely gaps from the inventory against the WINNING filter with projected totals

Example format for projected totals:
```
| Gap | Pain (C) | Timing (C) | Exec (U*) | Fit (U*) | Rev (U*) | Moat (U*) | Projected Total | Preliminary |
|-----|----------|------------|-----------|----------|----------|-----------|-----------------|-------------|
| SSO | 8 | 9 | 6* | 6* | 6* | 6* | 41/60 | FILE |
*U* = projected at 6/10; replace with your actual scores
```
